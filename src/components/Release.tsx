  ...
  return (
    <div className="card max-w-[150px] max-h-[150px] bg-base-100 shadow-xl image-full m-5">
      ...
      <div className="card-actions justify-center items-end">
        <label className="text-info btn btn-ghost btn-sm z-20 text-2xl" htmlFor="edit_release" onClick={() => setActiveRelease(release)}>
          <MdModeEdit />
        </label>
        <label className="text-error btn btn-ghost btn-sm z-20 text-2xl" htmlFor="delete_release" onClick={() => setActiveRelease(release)}>
          <MdDeleteForever />
        </label>
      </div>
    </div>
